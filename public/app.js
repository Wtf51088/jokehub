...
async function loadAdminPanel() {
  ...
  if (!response.ok) {
    document.getElementById("reportsList").innerHTML = `<p class="error">${escapeHTML(dashboard.error || "Admin Panel ვერ ჩაიტვირთა.")}</p>`;
    return;
  }

  const reports = dashboard.reports || [];
  const users = dashboard.users || [];
  const ipLogs = dashboard.ipLogs || [];
  const bannedIps = dashboard.bannedIps || [];
